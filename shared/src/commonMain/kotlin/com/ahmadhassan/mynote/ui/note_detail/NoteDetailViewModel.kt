...
class NoteDetailComponent(
    ...
    val viewModel = instanceKeeper.getOrCreate {
        NoteDetailViewModel(
            ...
            savedState = stateKeeper.consume(key = "viewModelState", NoteDetailViewModel.SavedState::class),
        )
    }
...
class NoteDetailViewModel(
    ...
    private val isContentTextFocused = MutableStateFlow(savedState?.isContentTextFocused ?: false)
    ...
    init {
        viewModelScope.launch {
            noteDataSource.getNote(noteId)?.let { note ->
                title.value = note.title
                content.value = note.content
                color.value = note.colorHex
            }
